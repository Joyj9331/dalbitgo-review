...
# 💡 @st.cache_data 삭제: 캐시를 없애서 접속할 때마다 무조건 최신 데이터를 읽어오게 만듭니다.
def load_data():
...
# 엑셀 파일이 없거나 읽지 못했을 경우, 오늘 수집된 데이터의 매장명으로 대체
...
if menu == "🌐 1. 전체 브랜드 평판 및 위험 감지":
    ...
    if not negative_df.empty:
        ...
        st.dataframe(negative_df[['매장명', '리뷰내용', '작성일']].reset_index(drop=True), use_container_width=True)
    else:
...
# ==========================================
    ...
    selected_store = st.selectbox("📌 분석하실 특정 가맹점을 선택하십시오 (엑셀 연동 완료)", full_store_list)
    ...
    if store_df.empty:
        st.success(f"✅ 오늘 '{selected_store}'에 새롭게 등록된 신규 리뷰가 없습니다. 평시와 동일하게 가맹점 관리를 진행해주시면 됩니다.")
        st.text_area(f"✍️ [{selected_store}] 슈퍼바이저 관리 메모 (점주 통화 내역 등)", placeholder="예: 오늘 신규 리뷰 없음. 다음 주 신메뉴 포스터 및 그릇류 발주 일정 공유 예정.")
    else:
        # 요약 지표 표시
